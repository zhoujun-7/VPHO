...
class DexYCBDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        obj_pose = torch.cat([obj_pose, torch.tensor([0, 0, 0, 1], dtype=torch.float32)], dim=0)

        obj_to_axsym_rt = torch.tensor(YCB_MESHES[obj_name]["shift"], dtype=torch.float32)
        obj_to_axsym_rt = torch.cat([obj_to_axsym_rt, torch.tensor([0, 0, 0, 1], dtype=torch.float32)], dim=0)
        obj_to_axsym_rt_inv = torch.inverse(obj_to_axsym_rt)
        obj_pose_from_axsym = obj_pose @ obj_to_axsym_rt_inv # from coordinate of object that origin at center of kpt3d of object to the hand wrist
        obj_pose_from_axsym = obj_pose[:3]
