...
from lib.utils.misc_fn import pt2d_to_bbox2d, expand_bbox2d, check_bbox2d
...
class DexYCBDataset(BaseDataset):
    ...
    def if_skip_sample(self, sample):
        if self.clean_data_mode == "2023_CVPR_HFL":
            ...
            is_skip = not filter_DexYCB_by_ArtiBoost(sample, filter_thresh=10)
    ...
    def __getitem__(self, index):
        index = self.index_ls[index] + ".pkl"
        sample = os.path.join(self.cache_dir, index)
        ...
        # region [spatial augmentation] *checked
        center_jittering, scale_factor, rot_factor = self.get_spatial_aug_params(self.is_train)
        rotmat_3d, rotmat_2d, cam_intrinsic = self.get_augmentation_rotmat(center_jittering, scale_factor, rot_factor, jt2d, obj_kpt2d, cam_intrinsic)
        ...
        _, _jt3d = get_hand_vert(mano_pose_aa_flat, mano_beta, mano_global_rot, mano_global_transl, is_right)
        ...
        # # region [bbox2d] #TODO: to be checked
        # bbox_hand = pt2d_to_bbox2d(jt2d, mode="x1y1x2y2")
        # bbox_hand = expand_bbox2d(bbox_hand, scale_factor=1.1)  # align with 2023_CVPR_HFL
        # check_bbox2d(bbox_hand, rgb_patch)
        # bbox_obj = pt2d_to_bbox2d(obj_kpt2d, mode="x1y1x2y2")
        # bbox_obj = expand_bbox2d(bbox_obj, scale_factor=1.0) # align with 2023_CVPR_HFL
        # check_bbox2d(bbox_obj, rgb_patch)

        # bbox_mask_hand = np.zeros_like(rgb_patch, dtype=np.bool_)
        # _bbox_hand = bbox_hand.astype(np.int32)
        # bbox_mask_hand[_bbox_hand[1]:_bbox_hand[3], _bbox_hand[0]:_bbox_hand[2]] = 1
        # _bbox_obj = bbox_obj.astype(np.int32)
        # bbox_mask_obj = np.zeros_like(rgb_patch, dtype=np.bool_)
        # bbox_mask_obj[_bbox_obj[1]:_bbox_obj[3], _bbox_obj[0]:_bbox_obj[2]] = 1
        # bbox_mask_unite = bbox_mask_hand + bbox_mask_obj
        # bbox_mask_inter = bbox_mask_hand * bbox_mask_obj
        # # endregion

        # # region [flip left hand]
        # if not is_right:
        #     rgb_flipped = rgb[:, ::-1].copy()
        ...
        #     #! correct translation 
        #     # TODO: correct translation by pa-align
        ...
        # hand_vert, jt3d = get_hand_vert(mano_pose_aa_flat, mano_beta, mano_global_rot, mano_global_transl, True)
        # # endregion
        ...
        rgb_tensor = self.image_augmentor.run_random_erasing(rgb_tensor) #* random erasing augmentation
        ...
        obj_6D[:3, 3] = obj_6D[:3, 3] - jt3d[0] #* to translation raletive to hand wrist
        ...
        # mano_params = np.concatenate([mano_global_rot, mano_pose_aa_flat, mano_beta]) # (58,)

        out = {
            "rgb_path": rgb_path,
            "rgb": rgb_tensor,
            "obj_pose": obj_pose,
            "root_joint": torch.tensor(jt3d[0], dtype=torch.float32),
