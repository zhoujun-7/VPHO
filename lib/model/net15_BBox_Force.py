...
class DiffHandObj_BBox_Force(nn.Module):
    def __init__(self):
        super(DiffHandObj_BBox_Force, self).__init__()
        self.cfg = cfg
        ...
        # self.cross_hand = CrossModule(8)
        # self.cross_obj = CrossModule(8)
        # self.head_physics = HeadPhysics()
    ...
    def forward(self, data, mode='predict'):
        ...
        if mode == 'train':
            ...
            pd_dt['mean_hand_joint'][idx_tensor] = get_joint_aligned_with_HO3D(pd_dt['mean_hand_vert'][idx_tensor], pd_dt['mean_hand_joint'][idx_tensor])
                # endregion [rank by heatmap, fuse topk]
            ...
            pd_dt['obj_final'] = obj_final.reshape(bs, self.cfg.sample_num, 9)
                ...
                # region [rank by heatmap and physics, fuse topk]
            hand_vert = pd_dt['mean_hand_vert'] + data['root_joint_flip'][:, None]
            force_point, force_global = self.head_physics.from_local_to_global(force_local=pd_phy_dt['force_local'], hand_vert=hand_vert)
            obj_select_data = self.obj_selector.select_topk_object(
                ...
                is_right=data['is_right'],
                heatmap=pd_hm_obj, # 3D points are flipped inside fn, thus the flipped heatmap is used
            ...
            pd_dt['obj_final'] = obj_select_data['pose6d_candidate']
                # endregion [rank by heatmap and physics, fuse topk]
            # endregion [diffusion generates object results]

            # # region [select hand by physics] # useless
            # hand_phy_sel_data = self.hand_selector.select_topk_hand_by_physics(
            #     pose_dif=pd_dt['hand_final'][..., :48],
            #     pose_reg=pd_mano_pose,
            #     pose_fuse=pd_dt['hand_mean'][..., :48],
            #     shape=pd_mano_shape,
            #     root_joint=data['root_joint_flip'], 
            #     cam_intrinsic=data['cam_intr_crop_flip'], 
            #     heatmap=pd_hm_hand, 
            #     bbox=data['bbox_hand'], 
            #     # k = min(self.cfg.topk_hand, 10),
            #     k = 15,
            #     force_local=pd_phy_dt['force_local'],
            #     obj_vert=obj_select_data['obj_vert_fused'],
            #     is_grasped=data['is_grasped'],
            # )
            
            # pd_dt['hand_mean'] = hand_phy_sel_data['fused_mano']
            # pd_dt['mean_hand_vert'], pd_dt['mean_hand_joint'] = hand_phy_sel_data['fused_vert'], hand_phy_sel_data['fused_joint']
            # pd_dt['mean_hand_joint'][idx_tensor] = get_joint_aligned_with_HO3D(pd_dt['mean_hand_vert'][idx_tensor], pd_dt['mean_hand_joint'][idx_tensor])
            # # endregion [select hand by physics]
