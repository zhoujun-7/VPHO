...
class DiffHandObj(nn.Module):
    ...
    def forward(self, data, mode='predict'):
        ...
        of_or_ls, pd_hm_obj_ori_ls = [], []
        for i, is_right_i in enumerate(data['is_right']):
            if not is_right_i:
                of_or_ls.append(of_or[i].flip(-1))
                pd_hm_obj_ori_ls.append(pd_hm_obj[i].flip(-1))
            else:
                of_or_ls.append(of_or[i])
                pd_hm_obj_ori_ls.append(pd_hm_obj[i])
        of_or = torch.stack(of_or_ls, dim=0)
        pd_hm_obj_ori = torch.stack(pd_hm_obj_ori_ls, dim=0)
        ...
        encoding_obj = self.encoder_obj(torch.cat((of_or, pd_hm_obj_ori_rs), dim=1)) # (bs, 1024)
