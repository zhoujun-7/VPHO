...
class DiffHandObj(nn.Module):
    ...
    def select_topk_hand(self, **kwargs):
        ...
        if kwargs['mode'] == 'heatmap_cascade':
            ...
            extra_pose = pose.clone().reshape(bs, -1, 48)
            extra_pose[:, :, 3:] = extra_pose[:, :, 3:] * 0 + kwargs['pose_regression'][:, None, 3:]
            ...
            for level_i in range(4):
                ...
                print(fused_data_i['fused_pose'].shape)
                exit()
                if level_i == 0:
                    pose = fused_data_i['fused_pose'][:, :self.cfg.sample_num+1].reshape(-1, 48)
                    shape = shape.reshape(bs, -1, 10)[:, :self.cfg.sample_num+1].reshape(-1, 10)
                else:
