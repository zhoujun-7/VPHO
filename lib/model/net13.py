...
from lib.model.physics import HeadForce, HeadForce2
...
class DiffHandObj_RegForce(nn.Module):
    def __init__(self):
        super(DiffHandObj_RegForce, self).__init__()
        self.cfg = cfg
        ...
        # self.head_force = HeadForce(256)
        # self.head_force = HeadForce2()
        ...
        # self.head_force.apply(init_weights)
    ...
    def forward(self, data, mode='predict'):
        ...
        of_or = flip_map_by_mask_index(of_or, is_flip=~data['is_right'])
        pd_hm_obj_ori = flip_map_by_mask_index(pd_hm_obj, is_flip=~data['is_right'])
        ...
        if hf_hr.shape != pd_hm_hand.shape:
            ...
            pd_hm_obj_ori_rs = F.interpolate(pd_hm_obj_ori, size=of_or.shape[-2:], mode='bilinear', align_corners=False)
        else:
            pd_hm_hand_rs, pd_hm_obj_ori_rs = pd_hm_hand, pd_hm_obj_ori
        encoding_hand, enc_hand_ls = self.encoder_hand(torch.cat((hf_hr, pd_hm_hand_rs), dim=1)) # (bs, 1024)
        encoding_obj, enc_obj_ls = self.encoder_obj(torch.cat((of_or, pd_hm_obj_ori_rs), dim=1)) # (bs, 1024)
        ...
        # feat_force = hf_hr
        # pd_force_local, scale = self.head_force(feat=feat_force, gravity=gravity_flipped)
        # pd_force_local, scale = self.head_force(enc_hand_ls=enc_hand_ls, enc_obj_ls=enc_obj_ls, gravity=gravity_flipped)
        # gt_force_point, pd_force_global = self.head_force.from_local_to_global(force_local=pd_force_local, hand_vert=data['gt_hand_vert_flip']) #! use gt data to train head_force when training
        # gt_point = torch.cat([gt_force_point, data['obj_CoM']], dim=1)
        ...
        if mode == 'train':
            ...
            # physics loss
            # physics_loss_dt = self.head_force.get_loss(pd_force_global=pd_force_global, gt_force_point=gt_force_point, 
            #                                            scale=scale, force_contact=data['force_contact'],
            #                                            gt_gravity=gravity_flipped, obj_CoM=obj_CoM_flipped, is_grasped=data['is_grasped'], 
            #                                            pd_force_local=pd_force_local, gt_force_local=data['force_local'],
            #                                         #    pd_point=pd_point, gt_point=gt_point,
            #                                            )
            ...
            hand_select_data = self.hand_selector.select_topk_hand(
                ...
                heatmap=pd_hm_hand, 
                bbox=data['bbox_hand'], 
                k=int(self.cfg.topk_hand*self.cfg.sample_num),
            )
            ...
            obj_select_data = self.obj_selector.select_topk_object(
                ...
                heatmap=pd_hm_obj, 
                bbox=data['bbox_obj'], 
                k=int(self.cfg.topk_obj*self.cfg.sample_num),
            )
            ...
            # region [physics]
            # pd_force_point, pd_force_global = self.head_force.from_local_to_global(force_local=pd_force_local, hand_vert=pd_dt['mean_hand_vert']) #! use pd data for evaluation
            # physics_dt = self.head_force.get_physics_metric(pd_force_global=pd_force_global, force_point=pd_force_point, 
            #                                                 gt_gravity=gravity_flipped, obj_CoM=obj_CoM_flipped, is_grasped=data['is_grasped'],
            #                                                 # pd_point=pd_point, gt_point=gt_point
            #                                                 ) # use gt gravity and obj center of mass for evaluation
            # pd_dt.update(physics_dt)
            # pd_dt['physics_metric'] = physics_dt
            # pd_dt['force_point'], pd_dt['force_global'] = pd_force_point, pd_force_global
            # pd_dt['force_local'] = pd_force_local
            # TODO: physics-based enhancement
            # endregion
            return pd_dt
...
def flip_map_by_mask_index(tensor, is_flip, dim=-1):
    flipped_ls = []
