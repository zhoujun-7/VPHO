...
from lib.model.encoding import Encoder2
...
class DiffHandObj_RegForce_Mutual(nn.Module):
    def __init__(self):
        super(DiffHandObj_RegForce_Mutual, self).__init__()
        self.cfg = cfg
        ...
        self.encoder_hand = Encoder2(256+21, 256, size_input_feature=(self.cfg.roi_size, self.cfg.roi_size))
        self.encoder_obj = Encoder2(256+27, 256, size_input_feature=(self.cfg.roi_size, self.cfg.roi_size))
    ...
    def forward(self, data, mode='predict'):
        ...
        of_or = flip_tensor_by_mask_index(of_or, is_flip=~data['is_right'])
        pd_hm_obj_ori = flip_tensor_by_mask_index(pd_hm_obj, is_flip=~data['is_right'])
        ...
        mid_enc_hand, enc_hand_ls = self.encoder_hand.stage1(torch.cat((hf_hr, pd_hm_hand_rs), dim=1)) # (bs, 1024)
        mid_enc_obj, enc_obj_ls = self.encoder_obj.stage1(torch.cat((of_or, pd_hm_obj_ori_rs), dim=1)) # (bs, 1024)
        ...
        cross_enc_hand, _, _ = self.cross_hand(enc_hand_ls[1], enc_obj_ls[1].detach(), gravity_flipped)
        _, cross_enc_obj, _ = self.cross_obj(enc_hand_ls[1].detach(), enc_obj_ls[1], gravity_flipped)
        cross_feat_hand = cross_enc_hand.transpose(0, 1).reshape(bs, 256, 8, 8)
        cross_feat_obj = cross_enc_obj.transpose(0, 1).reshape(bs, 256, 8, 8)

        # encoding_hand, enc_hand_ls = self.encoder_hand.stage2(cross_feat_hand, enc_hand_ls)
        # encoding_obj, enc_obj_ls = self.encoder_obj.stage2(cross_feat_obj, enc_obj_ls)
        encoding_hand, enc_hand_ls = self.encoder_hand.stage2(mid_enc_hand, enc_hand_ls)
        encoding_obj, enc_obj_ls = self.encoder_obj.stage2(mid_enc_obj, enc_obj_ls)
        ...
        pd_phy_dt = self.head_physics(cross_enc_hand, cross_enc_obj)
