...
class DiffHandObj(nn.Module):
    ...
    def forward(self, data, mode='predict'):
        ...
        encoding_hand, _ = self.encoder_hand(torch.cat((hf_hr, pd_hm_hand_rs), dim=1)) # (bs, 1024)
        encoding_obj, _ = self.encoder_obj(torch.cat((of_or, pd_hm_obj_ori_rs), dim=1)) # (bs, 1024)
        ...
        if mode == 'train':
            ...
            # region [joint2hm loss]
                # joint2hm_loss = self.head_mano.get_joint2hm_loss(
                #     pd_joint=pd_hand_joint, gt_joint=gt_hand_joint, root_joint=data['root_joint_flip'], cam_intr=data['cam_intr_crop_flip'], bbox=data['bbox_hand'], hm_size=64, sigma=4
                # )
                # loss_dt['joint2hm_loss'] = joint2hm_loss
            # endregion [joint2hm loss]
