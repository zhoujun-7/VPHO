...
class HandSelector:
    ...
    def select_topk_hand_by_observed_heatmap_and_fuse_by_index(self, **kwargs):
        ...
        if not kwargs['is_independent']: #* checked
            ...
            weight = val / val.sum(dim=1, keepdim=True) # (bs, K)
            ...
            weight = val / val.sum(dim=1, keepdim=True) # (bs, K, n)
...
class ObjectSelector:
    ...
    def select_topk_object(self, **kwargs):
        ...
        topk_trans2 = self.select_topk_object_by_heatmap(**kwargs)
        trans2_candidate = self.topk_select(topk_trans2, kwargs['pose6d']) # (bs, k, 9)
        trans2_candidate[:, :, :6] = trans2_candidate[:, :, :6] * 0
        trans2_fused = self.fuse_topk(topk=topk_trans2, **kwargs)[:, 6:]

        kwargs['pose6d'] = ori_pose6d.clone()
        kwargs['pose6d'][..., 6:] = ori_pose6d[..., 6:] * 0 + fused_trans1[:, None]
        topk_rot2 = self.select_topk_object_by_heatmap(**kwargs)
        rot2_candidate = self.topk_select(topk_rot2, kwargs['pose6d'])
        rot2_candidate[:, :, 6:] = rot2_candidate[:, :, 6:] * 0
        rot2_fused = self.fuse_topk(topk=topk_rot2, **kwargs)[:, :6]

        t2r2_fused = torch.cat([rot2_fused, trans2_fused], dim=-1)

        # region [force selection] #* useful, HO3Dv2-ADDS: 15.35 -> 14.15
        ...
        topk_physics = self.select_topk_object_by_physics2(**kwargs)
        pose6d_fused = self.fuse_topk(topk=topk_physics, **kwargs)

        ungrasp_idx = torch.arange(ori_pose6d.size(0), device=kwargs['pose6d'].device)
        ungrasp_idx = ungrasp_idx[~kwargs['is_grasped']]
        pose6d_fused[ungrasp_idx] = t2r2_fused[ungrasp_idx]
        # endregion
    ...
    def fuse_topk(self, **kwargs):
        ...
        rot6d_fused = average_rot6d(rot6d_topk)
