...
class DiffHandObj(nn.Module):
    ...
    def forward(self, data, mode='predict'):
        ...
        if mode == 'train':
            ...
            diff_hand_data = {'feat': encoding_hand, 'gt_pose': gt_mano_pose}
            ...
            k = 5
            diff_hand_joint2d = self.project_hand_point(
                joint3d_local=pd_dt['final_hand_joint'], 
                joint_root=data['root_joint_flip'], 
                cam_intrinsic=data['cam_intr_crop_flip'], 
            ) # (bs, sample_num, 21, 2)
            hand_topk = self.select_topk(
                mode='heatmap',
                pt2d=diff_hand_joint2d, 
                heatmap=pd_hm_hand, 
                bbox=data['bbox_hand'], 
                k=k
            )
            idx_tensor = torch.arange(bs, device=device)[:, None].repeat(1, k)
            hand_final_topk = pd_dt['hand_final'][idx_tensor, hand_topk]
            pd_dt['hand_mean'] = average_mano_params(hand_final_topk, mode='quat')
            pd_dt['final_hand_vert'] = pd_dt['final_hand_vert'][idx_tensor, hand_topk]
            pd_dt['final_hand_joint'] = pd_dt['final_hand_joint'][idx_tensor, hand_topk]
            # endregion

            pd_dt['mean_hand_vert'], pd_dt['mean_hand_joint'] = self.head_mano.get_hand_verts({'pose': pd_dt['hand_mean'][:, :48], 'shape': pd_dt['hand_mean'][:, 48:]})
    ...
    def project_hand_point(self, **kwargs):
        ''' project hand joint to 2D image plane
            joint3d_local: (bs, ..., 21, 3)
            joint_root: (bs, 3)
            cam_intrinsic: (bs, 3, 3)
            is_right: (bs,)
        '''
        shape = kwargs['joint3d_local'].shape
        expand_dim = len(shape) - 3
        joint3d_root = kwargs['joint_root'].view(shape[0], *([1]*expand_dim), 1, 3)
        joint3d_global= kwargs['joint3d_local'] + joint3d_root
        joint2d = torch.einsum('b...ij,blj->b...il', joint3d_global, kwargs['cam_intrinsic'])
        joint2d = joint2d[..., :2] / joint2d[..., 2:]
        return joint2d
        
    def select_topk(self, **kwargs):
        ...
        if kwargs['mode'] == 'heatmap':
            ...
            # TODO: cascade selection rather than the whole hand

        if kwargs['mode'] == 'hand_heatmap_cascade':
            ...
