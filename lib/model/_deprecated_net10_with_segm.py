...
class DiffHandObj(nn.Module):
    def __init__(self):
        ...
        self.head_segm_obj = HeadSegm(256, 1, 128, 1, 1)
        # self.head_hms_obj = HeadHeatmapSegm(256, 28, 128, 1, 1)

        self.encoder_hand = Encoder(256+21, 256, size_input_feature=(self.cfg.roi_size, self.cfg.roi_size))
        self.encoder_obj = Encoder(256+27+1, 256, size_input_feature=(self.cfg.roi_size, self.cfg.roi_size))
    ...
    def forward(self, data, mode='predict'):
        ...
        pd_segm_obj = self.head_segm_obj(of_or)
        # pd_hm_obj, pd_segm_obj = self.head_hms_obj(of_or)
        pd_hm_inplane = torch.cat([pd_hm_obj, pd_segm_obj], dim=1)

        #! flip back to original for object feature
        hf_or_ls, of_or_ls, pd_inplane_obj_ls = [], [], []
        for i, is_right_i in enumerate(data['is_right']):
            if not is_right_i:
                # hf_or_ls.append(hf_or[i].flip(-1))
                of_or_ls.append(of_or[i].flip(-1))
                pd_inplane_obj_ls.append(pd_hm_inplane[i].flip(-1))
            else:
                # hf_or_ls.append(hf_or[i])
                of_or_ls.append(of_or[i])
                pd_inplane_obj_ls.append(pd_hm_inplane[i])
        # hf_or = torch.stack(hf_or_ls, dim=0)
        of_or = torch.stack(of_or_ls, dim=0)
        pd_inplane_obj = torch.stack(pd_inplane_obj_ls, dim=0)
        ...
        if hf_hr.shape != pd_hm_hand.shape:
            ...
            pd_inplane_obj_rs = F.interpolate(pd_inplane_obj, size=of_or.shape[-2:], mode='bilinear', align_corners=False)
        else:
            pd_hm_hand_rs, pd_inplane_obj_rs = pd_hm_hand, pd_inplane_obj
        encoding_hand = self.encoder_hand(torch.cat((hf_hr, pd_hm_hand_rs), dim=1)) # (bs, 1024)
        encoding_obj = self.encoder_obj(torch.cat((of_or, pd_inplane_obj_rs), dim=1)) # (bs, 1024)
        ...
        if mode == 'train':
            ...
            loss_dt['segm_obj_loss'] = self.head_segm_obj.get_loss(pd_segm=pd_segm_obj, gt_segm=data['segm_obj'])
            # losses_obj_inplane = self.head_hms_obj.get_loss(pd_hm=pd_hm_obj, gt_hm=data['hm_obj'], pd_segm=pd_segm_obj, gt_segm=data['segm_obj'])
            # loss_dt.update(losses_obj_inplane)
            ...
            pd_dt['hand_vert'] = pd_hand_vert
            pd_dt['hand_joint'] = pd_hand_joint
            pd_dt['hand_contact'] = F.sigmoid(pd_hand_contact)
            pd_dt['hand_heatmap'] = pd_hm_hand
            pd_dt['obj_heatmap'] = pd_hm_obj
            pd_dt['obj_segm'] = F.sigmoid(pd_segm_obj)
            # pd_dt['hand_joint2d'] = pd_hand_joint2d
